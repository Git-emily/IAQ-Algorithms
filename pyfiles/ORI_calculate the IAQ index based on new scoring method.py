...
def IAQIndexCalculation(df_final, starting_hour):
    ...
    df_final['Week_Day'] = df_final['date_time'].map(
        lambda x: (pd.Timestamp(x.year, x.month, x.day, 0, 0, 0).weekday()) + 1)
    ...
    ending_date = starting_date + pd.Timedelta(days=7)
    ...
    i = 1
    while ending_date < max_time + pd.Timedelta(days=7):
        filtered_file = df_final[(starting_date <= df_final["date_time"]) &
                                 (df_final["date_time"] < ending_date)]
        filtered_file = filtered_file.sort_values(by='date_time', ascending=True)
        if len(filtered_file) != 0:
            ...
            filtered_file['Week_No'] = i
            file_list.append(list(filtered_file['Week_No']))
        ...
        starting_date += pd.Timedelta(days=7)
        ending_date += pd.Timedelta(days=7)
        i = i + 1

        print(starting_date)
        print(ending_date)
    for i in range(len(file_list)):
        LIST = LIST + file_list[i]
    df_final['Week_No'] = LIST
...
def Overall_visualization(df_final):
    """
    visualization
    Overall
    """
...
if __name__ == '__main__':
    ...
    df_final = Data_PreProcess(PATH + EcobeeData_name, PATH + '\\target unit(this is demo data)\\' + unit_name,
                               files_list)
    df_final = IAQIndexCalculation(df_final, curve_starting_hour)
    Overall_visualization(df_final)
