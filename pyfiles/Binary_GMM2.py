...
class Binary_GMM():
    def __init__(self, iter_time, data_array, Mu_1, Sigma_1, Mu_2, Sigma_2, Pi_weight, height, width):
    ...
    def E_step(self):
        ...
        gama_list = []
        for i in self.Data:
            a = self.PDF(i, self.mu_1, self.sigma_1)  # 概率密度函数
            b = self.PDF(i, self.mu_2, self.sigma_2)
            gama_i = (self.pw * a) / (self.pw * a + (1 - self.pw) * b)
    ...
    def M_step(self, gama_list):
        ...
        for i in range(len(gama_list)):
            new_mu_1 = new_mu_1 + self.Data[i] * gama_list[i] / N_1
            new_mu_2 = new_mu_2 + self.Data[i] * (1 - gama_list[i]) / N_2
        ...
        for i in range(len(gama_list)):
            ...
            vec_tmp.shape = (2, 1)
            new_sigma_1 = new_sigma_1 + np.dot((vec_tmp - new_mu_1), (vec_tmp - new_mu_1).transpose()) * gama_list[
                i] / N_1
        ...
        if self.iter_time == None:
            while (True):
                ...
                if math.fabs(delta_1[0][0]) < self.esp and math.fabs(delta_1[1][0]) < self.esp and math.fabs(
                        delta_2[0][0]) < self.esp and math.fabs(delta_2[1][0]) < self.esp:
                    break
        else:
...
if __name__ == "__main__":
    ...
    file_path1 = PATH + '\\Analysis_RH\\' + file_name1
    ...
    Bin_GMM = Binary_GMM(iter_time=None, data_array=dataMat1, Mu_1=Mu_1, Sigma_1=Sigma_1, Mu_2=Mu_2, Sigma_2=Sigma_2,
                         Pi_weight=Pi_weight, height=dataMat1.shape[0], width=dataMat1.shape[1])
