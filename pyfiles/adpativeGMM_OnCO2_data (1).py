    ...
    def updateParam(self, curFrame, BG_pivot):
        ...
        for i in range(self.height):
            for j in range(self.width):
                ...
                match=-1
                for k in range(self.numOfGauss):
                    CoVarInv=np.linalg.inv(self.sigmaSQs[i,j,k]*np.eye(1))
                ...
                if match!=-1:  ## a match found
                    ...
                    self.mus[i,j,-1]=X
                    self.sigmaSQs[i, j, -1] = 40000 # varVal or a high value
                    self.omegas[i,j,-1] = 0.02 # a low prior weight
                    labels[i,j]=250
                # re-normalizng the weights --- by Ziyou
                sum_omegas = sum(self.omegas[i,j])
                if(sum_omegas != 1.0):
                    for k in range(self.numOfGauss):
                        self.omegas[i,j,k] = self.omegas[i,j,k]/sum_omegas
    ...
    def extractCurrentBG(self):
        for i in range(self.height):
            for j in range(self.width):
                ...
                self.currentBG[i, j] = np.dot(self.omegas[i, j], self.mus[i, j, :])
